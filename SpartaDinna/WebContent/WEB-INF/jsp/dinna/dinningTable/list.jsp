...
<div id="container" class="container_24">
  ...
  <div id="content" class="container_24">
    <div class="container_24">
      <div class="area2">
        <!-- 分页显示演示开始 -->
        <form id="listForm" action="${ctx}/dinna/dinningTable/list" method="get">
          ...
          <table>
            <tr align="center">
              <th>id: <y:search fieldType="I" fieldList="id" matchType="EQ" />
                ...
                <a href="${ctx}/dinna/dinningTable/new">添加新dinningTable</a>
              </th>
            </tr>
          </table>
        </form>
      ...
      <div class="area">   
          <table  class="listtable">
            <tr class="line">
				<th><a href="javascript:sort('dtableId')">dtableId</a></th>
			
				<th><a href="javascript:sort('regionId')">regionId</a></th>
			
				<th><a href="javascript:sort('dtableName')">dtableName</a></th>
			
				<th><a href="javascript:sort('dtableState')">dtableState</a></th>
			
				<th><a href="javascript:sort('dtableDesc')">dtableDesc</a></th>
			
				<th><a href="javascript:sort('dtableRemark')">dtableRemark</a></th>
            ...
            <c:forEach var="entity" items="${pageData.result}">
              <tr <c:if test="${!entity.visible}">bgcolor="#AFEEEE"</c:if>>
					...
					<td>${entity.dtableId}</td>
				
					<td>${entity.regionId}</td>
				
					<td>${entity.dtableName}</td>
				
					<td>${entity.dtableState}</td>
				
					<td>${entity.dtableDesc}</td>
				
					<td>${entity.dtableRemark}</td>
                ...
                <td>
                	<a href="${ctx}/dinna/dinningTable/edit/${entity.id}">修改</a> |
                	<a href="${ctx}/dinna/dinningTable/delete/${entity.id}" onClick="return confirm('删除后无法恢复,确定要删除吗?');">刪除</a> |
                  <c:choose>
                    <c:when test="${entity.visible}"> 
                    	显示 |  <a href="${ctx}/dinna/dinningTable/unVisible/${entity.id}">不显示</a>
                    </c:when>
                    <c:otherwise> 
                    	<a href="${ctx}/dinna/dinningTable/visible/${entity.id}">显示</a> | 不显示 
                    </c:otherwise>
                  </c:choose>
                </td>
              </tr>
