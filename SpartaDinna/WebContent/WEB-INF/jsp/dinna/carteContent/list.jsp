...
<div id="container" class="container_24">
  ...
  <div id="content" class="container_24">
    <div class="container_24">
      <div class="area2">
        <!-- 分页显示演示开始 -->
        <form id="listForm" action="${ctx}/dinna/carteContent/list" method="get">
          ...
          <table>
            <tr align="center">
              <th>id: <y:search fieldType="I" fieldList="id" matchType="EQ" />
                ...
                <a href="${ctx}/dinna/carteContent/new">添加新carteContent</a>
              </th>
            </tr>
          </table>
        </form>
      ...
      <div class="area">   
          <table  class="listtable">
            <tr class="line">
				...
				<th><a href="javascript:sort('dish')">dish</a></th>
			
				<th><a href="javascript:sort('nums')">nums</a></th>
			
				<th><a href="javascript:sort('totalPrice')">totalPrice</a></th>
			
				<th><a href="javascript:sort('carteInfo')">carteInfo</a></th>
			
				<th><a href="javascript:sort('specialDescriprion')">specialDescriprion</a></th>
            ...
            <c:forEach var="entity" items="${pageData.result}">
              <tr <c:if test="${!entity.visible}">bgcolor="#AFEEEE"</c:if>>
					...
					<td>${entity.carteInfo}</td>
				
					<td>${entity.specialDescriprion}</td>
				
					<td>${entity.insertTime}</td>
                	<td>${entity.lastUpdateTime}</td>
                <td>
                	<a href="${ctx}/dinna/carteContent/edit/${entity.id}">修改</a> |
                	<a href="${ctx}/dinna/carteContent/delete/${entity.id}" onClick="return confirm('删除后无法恢复,确定要删除吗?');">刪除</a> |
                  <c:choose>
                    <c:when test="${entity.visible}"> 
                    	显示 |  <a href="${ctx}/dinna/carteContent/unVisible/${entity.id}">不显示</a>
                    </c:when>
                    <c:otherwise> 
                    	<a href="${ctx}/dinna/carteContent/visible/${entity.id}">显示</a> | 不显示 
                    </c:otherwise>
                  </c:choose>
                </td>
              </tr>
